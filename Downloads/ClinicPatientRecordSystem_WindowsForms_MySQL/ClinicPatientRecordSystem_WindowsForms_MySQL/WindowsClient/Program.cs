...
public class DashboardForm : Form
{
    ...
    private async Task RenderAppointmentsAsync()
    {
        ...
        var card = NewCard();
        var actions = new FlowLayoutPanel { Dock = DockStyle.Top, Height = 60, FlowDirection = FlowDirection.LeftToRight };
        ...
        foreach (Button b in actions.Controls) { b.Width = 130; b.Height = 42; }
        ...
        AppointmentDto? Selected() => grid.CurrentRow?.DataBoundItem as AppointmentDto;
        approve.Click += async (_, _) => { var a = Selected(); if (a != null) await UpdateAppointment(a.Id, "Approved", null); };
        cancel.Click += async (_, _) => { var a = Selected(); if (a != null) await UpdateAppointment(a.Id, "Cancelled", null); };
        resched.Click += async (_, _) =>
        {
            var a = Selected();
            if (a == null) return;
            var newDate = PromptDateTime(a.AppointmentDateTime);
            if (newDate.HasValue) await UpdateAppointment(a.Id, "Rescheduled", newDate.Value);
        };
    }
    ...
    private async Task UpdateAppointment(int id, string status, DateTime? newDate)
    {
        await RunSafe(async () =>
        {
            await ApiService.PutAsync($"appointments/{id}/status", new UpdateAppointmentStatusRequest(status, newDate));
    ...
    private async Task RenderDoctorsStaffAsync()
    {
        ...
        var card = NewCard();
        var top = new FlowLayoutPanel { Dock = DockStyle.Top, Height = 180, FlowDirection = FlowDirection.LeftToRight, WrapContents = false };
        card.Controls.Add(top);
        ...
        var tabs = new TabControl { Dock = DockStyle.Fill, Top = 290 };
        ...
        var doctorsGrid = Grid(); doctorsGrid.DataSource = doctors; doctorsGrid.Dock = DockStyle.Fill; doctorsTab.Controls.Add(doctorsGrid);
        var staffGrid = Grid(); staffGrid.DataSource = staffs; staffGrid.Dock = DockStyle.Fill; staffTab.Controls.Add(staffGrid);

        saveDoc.Click += async (_, _) => await RunSafe(async () =>
        {
            await ApiService.PostNoResultAsync("doctors", new Doctor(0, docName.Text, spec.Text, sched.Text));
            await RenderDoctorsStaffAsync();
        });
        saveStaff.Click += async (_, _) => await RunSafe(async () =>
        {
            await ApiService.PostNoResultAsync("staff", new StaffAccount(0, staffName.Text, pos.Text, user.Text));
            await RenderDoctorsStaffAsync();
        });
    }
    ...
    private async Task RenderReportsAsync()
    {
        ...
        var stats = new FlowLayoutPanel { Dock = DockStyle.Top, Height = 135, FlowDirection = FlowDirection.LeftToRight, WrapContents = true };
        ...
        var label = new Label { Text = "Diagnosis Summary", Dock = DockStyle.Top, Height = 40, Font = new Font("Segoe UI", 14, FontStyle.Bold), ForeColor = Color.FromArgb(5, 55, 70) };
