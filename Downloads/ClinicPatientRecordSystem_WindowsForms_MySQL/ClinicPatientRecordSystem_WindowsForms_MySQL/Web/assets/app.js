...
function patientsTable(patients) {
  return `<table class="table"><thead><tr><th>ID</th><th>Name</th><th>Contact</th><th>Address</th><th>Action</th></tr></thead><tbody>${patients.map(p => `<tr><td>${p.id}</td><td>${p.name}</td><td>${p.contact}</td><td>${p.address}</td><td class="actions"><button onclick="editPatient(${p.id})">Edit</button><button class="danger" onclick="deletePatient(${p.id})">Delete</button></td></tr>`).join('')}</tbody></table>`;
}
async function searchPatients() { setPanel('Search Results', patientsTable(await api(`/patients?search=${encodeURIComponent(search.value)}`))); }
function renderAddPatientAdmin() { setPanel('Add Patient', `${patientForm()}<button onclick="saveNewPatient().then(renderManagePatients)">Save Patient</button><p id="msg"></p>`); }
async function editPatient(id) { const p = await api(`/patients/${id}`); setPanel('Edit Patient', `${patientForm('', p)}<button onclick="saveEditedPatient(${id})">Save Changes</button><p id="msg"></p>`); }
async function saveEditedPatient(id) { await api(`/patients/${id}`, { method: 'PUT', body: JSON.stringify(readPatient()) }); renderManagePatients(); }
async function deletePatient(id) { if (confirm('Delete this patient?')) { await api(`/patients/${id}`, { method: 'DELETE' }); renderManagePatients(); } }

async function renderAppointmentsAdmin() {
  const appts = await api('/appointments');
  setPanel('Appointment Management', `<table class="table"><thead><tr><th>Patient</th><th>Doctor</th><th>Date</th><th>Reason</th><th>Status</th><th>Actions</th></tr></thead><tbody>${appts.map(a => `<tr><td>${a.patientName}</td><td>${a.doctorName}</td><td>${a.appointmentDateTime.replace('T',' ').substring(0,16)}</td><td>${a.reason}</td><td>${a.status}</td><td class="actions"><button onclick="setApptStatus(${a.id},'Approved')">Approve</button><button onclick="reschedule(${a.id})">Reschedule</button><button class="danger" onclick="setApptStatus(${a.id},'Cancelled')">Cancel</button></td></tr>`).join('')}</tbody></table>`);
}
async function setApptStatus(id, status, newDateTime = null) { await api(`/appointments/${id}/status`, { method: 'PUT', body: JSON.stringify({ status, newDateTime }) }); renderAppointmentsAdmin(); }
function reschedule(id) { const dt = prompt('New date/time, example: 2026-05-10T09:30'); if (dt) setApptStatus(id, 'Rescheduled', dt); }

async function renderDoctorsStaff() {
  const doctors = await api('/doctors'); const staff = await api('/staff');
  setPanel('Doctor & Staff Management', `<div class="grid"><div class="card"><h3>Add Doctor Schedule</h3><label>Name</label><input id="docName"><label>Specialization</label><input id="spec"><label>Schedule</label><input id="sched"><button onclick="addDoctor()">Save Doctor</button></div><div class="card"><h3>Add Staff</h3><label>Full Name</label><input id="staffName"><label>Position</label><input id="pos"><label>Username</label><input id="staffUser"><button onclick="addStaff()">Save Staff</button></div></div><h3>Doctors</h3><div class="cards">${doctors.map(d => `<div class="card"><h3>${d.name}</h3><p>${d.specialization}</p><p>${d.availableSchedule}</p></div>`).join('')}</div><h3>Staff</h3><div class="cards">${staff.map(s => `<div class="card"><h3>${s.fullName}</h3><p>${s.position}</p><p>${s.username}</p></div>`).join('')}</div>`);
}
async function addDoctor() { await api('/doctors', { method: 'POST', body: JSON.stringify({ name: docName.value, specialization: spec.value, availableSchedule: sched.value }) }); renderDoctorsStaff(); }
async function addStaff() { await api('/staff', { method: 'POST', body: JSON.stringify({ fullName: staffName.value, position: pos.value, username: staffUser.value }) }); renderDoctorsStaff(); }

async function renderReports() {
  const r = await api('/reports/summary');
  setPanel('Generate Reports', `<div class="cards"><div class="card"><h3>Daily Patients</h3><p>${r.dailyPatients}</p></div><div class="card"><h3>Total Patients</h3><p>${r.totalPatients}</p></div><div class="card"><h3>Pending Appointments</h3><p>${r.pendingAppointments}</p></div><div class="card"><h3>Approved Appointments</h3><p>${r.approvedAppointments}</p></div><div class="card"><h3>Income</h3><p>₱${r.income}</p></div></div><h3>Diagnosis Summary</h3><table class="table"><thead><tr><th>Diagnosis</th><th>Count</th></tr></thead><tbody>${r.diagnosisSummary.map(d => `<tr><td>${d.diagnosis}</td><td>${d.count}</td></tr>`).join('')}</tbody></table>`);
}
...
async function saveMedicalRecord() {
  try {
    const data = {
      ...
      recordDate: document.getElementById('mrDate').value,
    ...
    if (!data.patientId || !data.recordDate || !data.diagnosis || !data.prescription) {
      document.getElementById('msg').innerHTML =
